...
export async function getVenueBookings(
  ...
  return (data ?? []).map((booking: any) => ({
    ...
    status: booking.status as VenueBooking['status'],
    paymentStatus: booking.payment_status as VenueBooking['paymentStatus'],
...
export async function getVenueBookingMetrics(
  ...
  const todayBookings = (todayResult.data ?? []).length;
  ...
  const todayRevenue = (todayResult.data ?? [])
  ...
  const bookedHoursToday = (todayResult.data ?? [])
  ...
  return {
    ...
    todayRevenue,
    occupancyRate,
    availableHours: Math.max(0, totalOperatingHours - bookedHoursToday),
